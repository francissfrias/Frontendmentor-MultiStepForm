...
const Textfield = ({ className, label, ...props }: ITextfield) => {
  return (
    <div className={(clsx(className), 'mb-4')}>
      ...
      <input
        className='text-sm font-medium ring-1 ring-neutralLightGray rounded-md pt-2 pb-3 px-4  w-full  focus:outline-none focus:ring-1 focus:ring-primaryPurplishBlue'
        {...props}
      />
    </div>
  );
